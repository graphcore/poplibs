...
fields = [
    ...
    ('IPU exchange cycles',
     'IPU exchange cycles\s*:\s*(?P<value>[0-9.]+)'),
    ('Send instruction bytes',
     'Send:\s*(?P<value>[0-9.]+) bytes'),
    ('Send with delay instruction bytes',
     'Send with delay:\s*(?P<value>[0-9.]+) bytes'),
    ('Send with put mux instruction bytes',
     'Send with put mux:\s*(?P<value>[0-9.]+) bytes'),
    ('Send with put address instruction bytes',
     'Send with put address:\s*(?P<value>[0-9.]+) bytes'),
    ('Send with put format instruction bytes',
     'Send with put format:\s*(?P<value>[0-9.]+) bytes'),
    ('Receive mux instruction bytes',
     'Receive mux:\s*(?P<value>[0-9.]+) bytes'),
    ('Receive address instruction bytes',
     'Receive address:\s*(?P<value>[0-9.]+) bytes'),
    ('Receive format instruction bytes',
     'Receive format:\s*(?P<value>[0-9.]+) bytes'),
    ('Exchange instruction bytes',
     'Exchange:\s*(?P<value>[0-9.]+) bytes'),
    ('Delay instruction bytes',
     'Delay:\s*(?P<value>[0-9.]+) bytes'),
    ('Tile sync',
    ...
    ('Average bytes received',
     'Average bytes received per cycle\s*:\s*(?P<value>[0-9.]+)'),
    ('Average bytes sent',
     'Average bytes sent per cycle\s*:\s*(?P<value>[0-9.]+)'),
    ('FLOPS',
...
def write(runs, filename):
    ...
    headings2 = [
        ...
        ('IPU exchange cycles', setDataCell),
        ('Tile sync', setDataCell),
        ('IPU sync', setDataCell),
        ('Global sync', setDataCell),
        ('Total cycle count', partial(setTotalCell, -6, -1, 'row')),
        ('Average bytes sent', setDataCell),
        ('Average bytes received', setDataCell),
        ('Send instruction bytes', setDataCell),
        ('Send with delay instruction bytes', setDataCell),
        ('Send with put mux instruction bytes', setDataCell),
        ('Send with put address instruction bytes', setDataCell),
        ('Send with put format instruction bytes', setDataCell),
        ('Receive mux instruction bytes', setDataCell),
        ('Receive address instruction bytes', setDataCell),
        ('Receive format instruction bytes', setDataCell),
        ('Exchange instruction bytes', setDataCell),
        ('Delay instruction bytes', setDataCell),
        ('Exchange supervisor code', partial(setTotalCell, -10, -1, 'row')),
        ('Message memory', setDataCell),
        ('Run instructions', setDataCell),

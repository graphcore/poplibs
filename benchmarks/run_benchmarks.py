...
fields = [
    ...
    ('Send with put instruction bytes',
     'Send with put:\s*(?P<value>[0-9.]+) bytes'),
    ('Receive instruction bytes',
     'Receive:\s*(?P<value>[0-9.]+) bytes'),
...
def write(runs, filename):
    ...
    headings2 = [
        ...
        ('Send with put instruction bytes', setDataCell),
        ('Receive instruction bytes', setDataCell),
        ('Exchange instruction bytes', setDataCell),
        ('Delay instruction bytes', setDataCell),
        ('Exchange supervisor code', partial(setTotalCell, -6, -1, 'row')),
        ('Message memory', setDataCell),
        ('Run instructions', setDataCell),

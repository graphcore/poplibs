...
// Create and map the per channel multiplicative gamma parameter tensor used for
// normalisation in convolution layers.
// \param graph           The graph to which the activations and gamma tensor
//                        belong
...
// normalisation in convolution layers
// \param graph           The graph in which the activations and beta tensor
//                        belong
// \param acts            The activations tensor has shape [N][C][..F..]
//                          where N is the batch size
//                           C is the number of channels
...
// \param graph           The graph to which the activations and beta/gamma
//                        tensors belong
// \param acts            The activations tensor has shape [N][C][..F..]
//                          where N is the batch size
//                           C is the number of channels
...
// activations tensor is of shape [N][C][..F..]. The mean and inverse std
// deviation is computed over dimensions {[N] [..F..]| and vectors of length C
// are returned as estimates.
// The input activations tensor must be rearranged such that statistics are
// computed for C channels.
// \param graph          The graph in which the computation is in
// \param actsUngrouped  The activation with shape [N][C][..F..]
// \param eps            The epsilon added to the variance to avoid divide by 0
...
std::pair<poplar::Tensor, poplar::Tensor>
normStatistics(poplar::Graph &graph,
...
// deviation. The input activations undergo a prior rearrangement such that C
...
// \param debugPrefix     A debug prefix added to compute set and tensor names
std::pair<poplar::Tensor, poplar::Tensor>
normParamGradients(poplar::Graph &graph,
                   const poplar::Tensor &actsWhitened,
...
// \param debugPrefix     A debug prefix added to compute set and tensor names
poplar::Tensor
normGradients(poplar::Graph &graph,
              const poplar::Tensor &gradsIn,
              const poplar::Tensor &gamma,
              poplar::program::Sequence &prog,
              const std::string &debugPrefix = "");
...
// activations and the input gradients must have undergone a prior rearrangement
// such that the channel dimension has the same elements as invStdDev.
// \param graph           The graph in which the normalisaton operation is added
...
// \param debugPrefix     A debug prefix added to compute set and tensor names
poplar::Tensor
normStatisticsGradients(poplar::Graph &graph,
                        const poplar::Tensor &actsWhitened,
                        const poplar::Tensor &gradsIn,

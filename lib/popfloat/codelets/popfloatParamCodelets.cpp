...
public:
  ...
  bool compute() {
    ...
    gf16CalmpOut[POPFLOAT_IPU_CLAMP_INDEX_MIN] = hlfCalmpOut.bit16() |
                                                 POPFLOAT_FP16_SIGN_MASK;
...
public:
  ...
  bool compute() {
    ...
    float maxValue;
    uintAsVec<float, uint32_t, 1>(
       &maxValue, POPFLOAT_FP32_POWER2(maxExpGf32) | gf32ManMask);
    ...
    std::memcpy(&param[POPFLOAT_CAST_TO_GF32_PARAM_HALF_MIN_OFFSET],
                &gf32HalfMin,
                sizeof(gf32HalfMin));
    std::memcpy(&param[POPFLOAT_CAST_TO_GF32_PARAM_MIN_VALUE_OFFSET],
                &gf32Min,
                sizeof(gf32Min));

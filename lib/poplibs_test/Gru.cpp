 ...
 */
static void
processBasicGruUnit(const Array2dRef        prevOutput,
  ...
  gemm::generalMatrixMultiply(prevOutput, weightsOutputUnit, output,
                              false, false);
...
void poplibs_test::gru::
basicGruCellForwardPass(const Array3dRef input,
                        const Array2dRef biases,
  ...
  for (auto s = 0U; s != sequenceSize; ++s) {
    ...
    processBasicGruUnit(tmp1, inputThisStep, weightsInput,
...
static Array2d
concatMatrix2D(const Array2d matA,
               const Array2d matB,
                         int dimension)
{
...
void poplibs_test::gru::
basicGruCellBackwardPass(      bool       outputFullSequence,
                         const Array3dRef weightsInput,
                         const Array3dRef weightsOutput,
  ...
  Array2d w_ru = concatMatrix2D(
           ...
           concatMatrix2D(weightsInput[GRU_FWD_STATE_UPDATE_GATE_IDX],
                          weightsOutput[GRU_FWD_STATE_UPDATE_GATE_IDX], 0), 1);
  FILE *fp = NULL;
#ifdef DEBUG_TENSOR
  fp = fopen("bwd.txt", "w");
#endif
...
void poplibs_test::gru::
basicGruCellParamUpdate(const Array3dRef prevLayerActs,
                        const Array4dRef fwdState,
                        const Array2dRef outputActsInit,

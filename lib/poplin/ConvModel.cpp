  ...
  auto visitor = poplibs_support::make_visitor<popsolver::Variable>(
        ...
        return m.call<unsigned>(
            ...
            [&target, params, fieldGrainSize, convGroupsPerGroup,
             ...
             transformedOutputStride, method, actsType, floatPartials, cache](
              ...
              // we process kernel width in 1x4 blocks (rounding up to the
              // nearest multiple of the SLIC kernel width) and then do this
              // for each other kernel dimension.
              const unsigned numWeightBlocks = [&] {
                ...
                const unsigned otherDims = product(convSize.kernelSize) /
                                           convSize.kernelSize[widthDim];
            ...
            [&target, fieldGrainSize, inChansPerGroup, convGroupsPerGroup,
             ...
             actsType, floatActivations, floatPartials](
              ...
              for (unsigned dim = 0; dim + 1 < numFieldDims; ++dim) {
                const auto dimActiveRows = (convSize.fieldSize[dim] +
                                            transformedInputDilation[dim] - 1) /
      ...
      [&](const Plan::Vmac &method) {
        return m.call<unsigned>(
            convSizeVarsVector,
            [&target, fieldGrainSize, inChansPerGroup, convGroupsPerGroup,
             outChansPerGroup, transformedInputDilation, cache,
             floatActivations, floatPartials,
        ...
        return m.call<unsigned>(
            ...
            [fieldGrainSize, numContexts, convGroupsPerGroup, outChansPerGroup,
             ...
             dataPathWidth](const std::vector<unsigned> &values)
              ...
              // OuterProduct currently only expects a single convGroup
              // grouping.
...
addConvTempMemoryEstimate(popsolver::Model &m,
  ...
  auto visitor = poplibs_support::make_visitor<void>(
      [&](const Plan::Slic &) {
        const auto buffer = m.sum(
            {m.ceildiv(partialStorage, convSizes.back().numConvGroupGrains),
             m.addConstant(200)});
        partialStorage = m.sum({partialStorage, buffer});
      },
      [&](const auto &) {});
  boost::apply_visitor(visitor, method);
  ...
  // convolution.
  return std::make_tuple(inputStorage, weightStorage, partialStorage);
}
...
padKernelSpatialDim(popsolver::Model &m, const ConvParams &params,
  ...
  assert(transformedSizes.size() >= numLevels);
  ...
  assert(transformedSizes[systemLevel].kernelSize.size() > dim);
  assert(partitionVars[systemLevel].kernelSplit.size() > dim);
  ...
  const auto h = transformedSizes[systemLevel].kernelSize[dim];
  const auto s = partitionVars[systemLevel].kernelSplit[dim];
...
static void truncateDilateAndPadInput(
  ...
  const auto fieldGrainSize =
      m.addConstant(partitionVars[systemLevel].fieldGrainSize[dim]);
...
applyPadding(popsolver::Model &m, const poplar::Target &target,
  ...
  const auto convGroupSize =
      m.product({transformedSizes[tileLevel].numConvGroupGrains,
                 m.addConstant(partitionVars[systemLevel].convGroupGrainSize)});
  ...
  const auto inChanSize =
      m.product({transformedSizes[tileLevel].numInChanGrains,
                 m.addConstant(partitionVars[systemLevel].inChanGrainSize)});
  const auto outChanSize =
      m.product({transformedSizes[tileLevel].numOutChanGrains,
                 m.addConstant(partitionVars[systemLevel].outChanGrainSize)});
  ...
  const auto kernelCycles = exchangeEstimator.getCycles(kernelZeros, inputType);
  const auto inputCycles =
      exchangeEstimator.getInputElementCycles(inputZeros, inputType);
...
addTileLevelTransformEstimates(
  ...
  auto visitor = poplibs_support::make_visitor<
      std::pair<popsolver::Variable, popsolver::Variable>>(
      [&](const Plan::Hmac &) { return std::make_pair(zero, zero); },
      [&](const Plan::Vmac &) { return std::make_pair(zero, zero); },
      [&](const Plan::OuterProduct &) { return std::make_pair(zero, zero); },
      [&](const Plan::Slic &method) {
        ...
        assert(method.windowWidth == 4);

        // SLIC pads the kernel width dimension which is the innermost spatial
        // dim.
        const unsigned dimToPad = params.kernelShape.size() - 1;
        padKernelSpatialDim(m, params, transformedSizes, partitionVars,
                            kernelPadding, inputPadding, method.windowWidth,
                            dimToPad);
      ...
      [&](const Plan::Amp &method) {
        ...
        // outer-most spatial dimension, it then uses that dimension so make up
        // for the lack of input channels.
...
ExchangeEstimates<popsolver::Variable> addExchangeCycleEstimates(
  ...
  // weights and partials at the system level. These are stored in each vector.
  // The sum of each vector is returned to give itemised results and help with
  // analysis.
  ...
  for (unsigned level = 0; level != numLevelsOfHierarchy - 1; ++level) {
    ...
    inputExchangeCycles.push_back(exchangeEstimator.getInputElementCycles(
        numberOfInputElements, params.inputType));

    weightExchangeCycles.push_back(
        exchangeEstimator.getCycles(numberOfWeights, params.inputType));
    ...
    reduceFirstStageExchangeCycles.push_back(exchangeEstimator.getCycles(
        numberOfPartialSums, types[level + 1].resultType));
...
static TransformEstimates<popsolver::Variable> addTransformCycleEstimate(
  ...
  assert(transforms.size() >= numLevels);
  for (unsigned level = 0; level <= systemLevel; ++level) {
  ...
  std::vector<popsolver::Variable> ipuSplits = {
      partitionVars[systemLevel].batchSplit,
      partitionVars[systemLevel].convGroupSplit,
      partitionVars[systemLevel].inChanSplit.parallel,
      partitionVars[systemLevel].outChanSplit.parallel};
  ipuSplits.insert(ipuSplits.end(),
                   partitionVars[systemLevel].fieldSplit.begin(),
                   partitionVars[systemLevel].fieldSplit.end());
  ipuSplits.insert(ipuSplits.end(),
                   partitionVars[systemLevel].kernelSplit.begin(),
                   partitionVars[systemLevel].kernelSplit.end());
  ...
  if (rearrangeOutput || regroupOutput) {
    ...
    const auto outputBytesPerElement =
        target.getTypeSize(types[systemLevel].resultType);
    auto bytesPerTile =
...
addRearrangeBeforeSliceEstimate(popsolver::Model &m,
  ...
  const auto exchangeCycles =
      exchangeEstimator.getCycles(weightsPerTile, params.inputType);
...
static SinglePassEstimates<popsolver::Variable> addEstimates(
    ...
    const std::vector<std::unordered_set<unsigned>> &transformedDims,
    const poplar::Target &target, const ConvParams &untransformedParams,
    const ConvParams &transformedViewParams,
    const ConvParams &transformedOnceParams,
  ...
  ExchangeEstimator exchangeEstimator(m, target, partitionVars,
                                      linearizeTileOrder);
  ...
  popsolver::Variable inputsConvTempMemory;
  popsolver::Variable weightsConvTempMemory;
  popsolver::Variable partialsConvTempMemory;
  std::tie(inputsConvTempMemory, weightsConvTempMemory,
           partialsConvTempMemory) =
  ...
  std::tie(e.rearrangeBeforeSliceCycles, e.rearrangeBeforeSliceTempBytes,
      ...
      addRearrangeBeforeSliceEstimate(
          ...
          systemLevel, transformedOnceParams, options, isJointPlan);
  ...
  e.broadcastInputBeforeLoopTempBytes =
      m.product({m.sum({e.itemisedTransformEstimates.inputsTempBytes,
                        inputsConvTempMemory}),
                 broadcastInputBeforeConvLoop});
  ...
  e.itemisedExchangeCycles.inputExchangeCycles =
      m.product({e.itemisedExchangeCycles.inputExchangeCycles,
                 broadcastInputInsideConvLoop});
  inputsConvTempMemory =
      m.product({inputsConvTempMemory, broadcastInputInsideConvLoop});
  ...
  // Get total estimate of in loop temp memory
  e.convTempBytes = m.sum(
      {inputsConvTempMemory, weightsConvTempMemory, partialsConvTempMemory},
      "tempConvBytes");
...
static Plan::Method getFullyConnectedBwdMethod(Plan::Method fwdMethod) {
  return boost::apply_visitor(
      poplibs_support::make_visitor<Plan::Method>(
          [&](const Plan::OuterProduct &) { return Plan::Hmac{}; },
          [&](const auto &method) { return method; }),
      fwdMethod);
}
...
std::string toPTreeTypeStr(const Plan::Method &m) {
  const auto visitor = poplibs_support::make_visitor<std::string>(
      [&](const Plan::Hmac &m) { return "HMAC"; },
      [&](const Plan::Vmac &) { return "VMAC"; },
      [&](const Plan::Amp &m) { return "AMP"; },
      [&](const Plan::Slic &m) { return "SLIC"; },
      [&](const Plan::OuterProduct &) { return "OUTER_PRODUCT"; });
...
ConvVertexType getFullyConnectedBwdConvVertexType(
  ...
  if (bwdMethod.type() == typeid(Plan::Vmac) ||
      bwdMethod.type() == typeid(Plan::Hmac)) {
    return ConvVertexType(bwdMethod, bwdParams.inputType,
                          bwdOptions.partialsType, 1, bwdInChansPerGroup,
                          bwdOutChansPerGroup);
  }
  bwdOptions.planConstraints.put("method.type", toPTreeTypeStr(bwdMethod));
  if (fwdMethod.type() == typeid(Plan::Amp) &&
      bwdMethod.type() == typeid(Plan::Amp)) {
...
addBwdEstimates(popsolver::Model &m, const ConvParams &fwdUntransformedParams,
  ...
  return addEstimates(
      ...
      transformedDims, target, bwdUntransformedParams, bwdTransformedViewParams,
      bwdTransformedOnceParams, bwdTransformedOnceUnpaddedParams, isJointPlan,
      bwdConvVertexType, broadcastInputBeforeLoop, bwdTypes, transforms,
      Plan::LinearizeTileOrder::FC_BWD_AS_CONV, referenceCost, bwdOptions,
      cache);
}
...
static Plan::Method getFullyConnectedWUMethod(const ConvParams &wuParams,
  ...
  auto visitor = poplibs_support::make_visitor<Plan::Method>(
      [&](const Plan::OuterProduct &method) { return Plan::Hmac{}; },
      [&](const auto &method) { return method; });
  return boost::apply_visitor(visitor, fwdMethod);
}
...
ConvVertexType getFullyConnectedWuConvVertexType(
  ...
  if (wuMethod.type() == typeid(Plan::Vmac) ||
      wuMethod.type() == typeid(Plan::Hmac)) {
    return ConvVertexType(wuMethod, wuParams.inputType, wuOptions.partialsType,
                          1, fwdOutChansPerGroup, fwdInChansPerGroup);
  }
  wuOptions.planConstraints.put("method.type", toPTreeTypeStr(wuMethod));
  if ((fwdMethod.type() == typeid(Plan::Amp) &&
       wuMethod.type() == typeid(Plan::Amp)) ||
      wuMethod.type() == typeid(Plan::OuterProduct)) {
...
addWuEstimates(popsolver::Model &m, const ConvParams &fwdUntransformedParams,
  ...
  return addEstimates(
      ...
      transformedDims, target, wuUntransformedParams, wuTransformedViewParams,
      wuTransformedOnceParams, wuTransformedOnceUnpaddedParams, isJointPlan,
      wuConvVertexType, broadcastInputBeforeLoop, wuTypes, transforms,
...
static void addMethodConstraints(popsolver::Model &m,
                                 const Plan::Method &method,
                                 const PartitionVariables &p,
                                 const ConvSizeVariables &s,
                                 const ConvParams &lvl1Params) {
...
getUsedTiles(popsolver::Model &m,
             const std::vector<PartitionVariables> &partitionVars,
             const unsigned &tilesPerIPU) {
  popsolver::Variable perLevelSplits;
  ...
  perLevelSplits = m.product(splits, arrIndStr(0) + ".partition.total");
  m.lessOrEqual(perLevelSplits, popsolver::DataType{tilesPerIPU});

  return perLevelSplits;
}
...
Estimates<popsolver::Variable> constructModel(
  ...
  // other words how many tiles in the tile split. we add one level of hierarchy
  // here to represent the whole system level which comes before the tile split
  // level. each level only supports certain transforms and the tile level has
  // no partition splits as it is the last level (so there is nothing to split
  // into).

  const auto numTilesPerIPU = target.getTilesPerIPU();
  const auto numLevelsOfHierarchy = numLevels;
  partitionVars.clear();
  ...
  for (unsigned level = 0; level != numLevelsOfHierarchy; ++level) {
    ...
    convSize.back().numFieldGrains.reserve(numFieldDims);
    convSize.back().kernelSize.reserve(numFieldDims);
    const auto levelMaxSplit = numTilesPerIPU;
    ...
    // We only support splitting serially in the system level of the hierarchy.
    // This is always the ultimate level.
    // TODO: T10037 For now we do not attempt to serially split any plan
    // that has an inter-IPU level split.
    assert(numLevelsOfHierarchy >= numLevels);
    if (numLevelsOfHierarchy == numLevels && level == systemLevel) {
...

  {
    ...
    addMethodConstraints(m, convVertexType.method, partitionVars[systemLevel],
                         convSize[tileLevel], transformedOnceParams);
  }

  const auto usedTiles = getUsedTiles(m, partitionVars, numTilesPerIPU);
  ...
  e.passEstimates = addEstimates(
      ...
      transformedDims, target, untransformedParams, transformedViewParams,
      transformedOnceParams, transformedOnceUnpaddedParams, isJointPlan,
      convVertexType, broadcastInputBeforeLoop, types, transforms,
  ...
  if (isJointPlan) {
    ...
    e.jointPlanBwdEstimates = addBwdEstimates(
        ...
        usedTiles, target, isJointPlan, bwdReferenceCost, options, cache);

    e.jointPlanWuEstimates = addWuEstimates(
        m, untransformedParams, transformedViewParams, transformedOnceParams,
        transformedOnceUnpaddedParams, numLevelsOfHierarchy, partitionVars,
        convSize, transforms, convVertexType, broadcastInputBeforeLoop,
        usedTiles, target, numFieldDims, isJointPlan, wuReferenceCost, options,
        cache);

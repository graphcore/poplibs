...
static Estimates<popsolver::Variable> addEstimates(
  ...
  if (referenceCost) {
    const auto posDiff = [&m](popsolver::Variable lhs, int rhs) {
      // can't use Model::sub here because that will invalidate the plan if the
      // answer is negative.
      return m.call({lhs}, [rhs](const std::vector<unsigned> &vs) -> unsigned {
        return std::max(0, int(vs[0]) - rhs);
      });
    };
...
getConvGroupGrainSizes(const std::vector<ConvTransform> &transforms,
  ...
  // unless these is a transform that rearranges the group axis (of which there
  // are currently none that exist).

...
public:
  ...
  bool compute() {
    ...
    int unpackedTransformedInRowStride = transformedInRowStride;
    int unpackedTransformedInStride = transformedInStride;
    int unpackedTransformedOutStride = transformedOutStride;

    if (useLimitedVer) {
      const int convOutputStoreElems = std::is_same<AccumType, half>() ? 4 : 2;

      unpackedTransformedInRowStride =
          unpackAmpNx1InRowStride(ampKernelHeight, transformedInRowStride);
      unpackedTransformedInStride =
          unpackAmpNx1Stride(NUM_STRIDE_BITS, transformedInStride, 1);
      unpackedTransformedOutStride =
          unpackAmpNx1Stride(NUM_STRIDE_BITS, transformedOutStride, 0);

      unpackedTransformedInStride = reverseTransformedInStrideNx1(
          unpackedTransformedInStride, transformedInRowStride);

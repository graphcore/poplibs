...
static Tensor
basicGruUnitsNlInput(Graph &graph,
                      Tensor prevAct,
                      Tensor prevOutput,
                      size_t num_unit,
                      Tensor weightsInput,
...
GruParams::GruParams(poplar::Type dataType,
...
void zeroInitialState(Graph &graph, const Tensor &init_output,
...
createWeightsKernel(poplar::Graph &graph, const GruParams &params,
  ...
  auto weights_ru =
      createMatMulInputRHS(graph, params.dataType,
                           ...
                           name + "/weights",
                           mmOpt, cache);
  poplar::Tensor inputWeights_ru = unflattenUnits(weights_ru.slice(0,
                                       inputSize), 2);
  poplar::Tensor outputWeights_ru = unflattenUnits(weights_ru.slice(inputSize,
  ...
  auto weights_c =
      createMatMulInputRHS(graph, params.dataType,
                           ...
                           name + "/weights",
                           mmOpt, cache);
  poplar::Tensor inputWeights_c = unflattenUnits(weights_c.slice(0, inputSize),
                                1);
  poplar::Tensor outputWeights_c = unflattenUnits(weights_c.slice(inputSize,
...
GruWeights
createWeights(Graph &graph, const GruParams &params,
              const std::string &name,
              const OptionFlags &options,
              poplin::matmul::PlanningCache *cache) {
...
gruCellForwardPassCalcUnits(Graph &graph,
  ...
  auto bBiases = graph.addVariable(dType, {0, batchSize, outputSize},
                                   "bbiases");
  if (forCandidate) {
    auto unitBias = biases[2].broadcast(batchSize, 0)
  ...
  rearrangeUnitsOutputFwd(graph, numUnit, unitsOutput,
  ...
  if (forCandidate) {
    ...
    nonLinearityInPlace(graph, popnn::NonLinearityType::SIGMOID,
               unitsOutputRearranged, cs, baseStr + "update/reset sigmod");
  }
  prog.add(Execute(cs));
}

static std::pair<Tensor, GruInternalState>
basicGruCellForwardPass(Graph &graph,
                        const Tensor &in,
                        const Tensor &biases,
                        const Tensor &prevOutput,
                        const Tensor *weightsInput,
  ...
  toConcat.push_back(
    graph.clone(prevOutput,
                debugPrefix + "/" +
                "Update Gate Rearranged").expand({0})
  );
  toConcat.push_back(
    graph.clone(prevOutput,
                debugPrefix + "/" +
                "Reset Gate Rearranged").expand({0})
  );
  ...
  Tensor resetGateOut = graph.clone(resetGate,
                                    debugPrefix + "/" + "resetGateOut");
  prog.add(Copy(resetGate, resetGateOut));
  auto updateGate = unitsOutput[BASIC_GRU_CELL_UPDATE_GATE];

  Tensor candidate = graph.clone(prevOutput,
                                 debugPrefix + "/" + "candidate Rearranged");
...
static void
basicGruCellForwardPassInPlace(Graph &graph,
                                const Tensor &in,
                                const Tensor &biases,
                                const Tensor &output,
                                const Tensor *weightsInput,
  ...
  Tensor candidate = graph.clone(output,
                                 debugPrefix + "/" + "candidate Rearranged");
  const Tensor weightsInput3  = weightsInput->slice(2, 3);
  const Tensor weightsOutput3 = weightsOutput.slice(2, 3);
...
gruFwd(Graph &graph,
  ...
  Tensor output = duplicate(graph, fwdOutputInit, fwdProg,
              debugPrefix + "/fwdOutput");
  ...
  if (intermediatesSeq) {
    ...
    *intermediatesSeq =
    ...
    popops::dynamicUpdate(
        ...
        {0}, {1}, loop, debugPrefix + "/gruUpdateIntermediates");
  ...
  if (params.outputFullSequence) {
    ...
    popops::dynamicUpdate(
        graph, outputSeq, output.expand({0}), seqIdx, {0}, {1},
...
backwardStepImpl(Graph &graph,
  ...
  auto u =
      fwdIntermediates[GRU_FWD_INTERMEDIATE_UPDATE_GATE];
  auto r =
      fwdIntermediates[GRU_FWD_INTERMEDIATE_RESET_GATE];
  auto c =
      fwdIntermediates[GRU_FWD_INTERMEDIATE_CANDIDATE];
  auto h_prev = prevStepOut;

  auto one_matrix = graph.addConstant(outputGrad.elementType(),
                              outputGrad.shape(), 1, fPrefix + "/one_matrix");
  graph.setTileMapping(one_matrix, graph.getTileMapping(u));
  auto var_one_matrix = graph.addVariable(outputGrad.elementType(),
                          outputGrad.shape(), fPrefix + "/var_one_matrix");
  ...
  // u_com = 1 - u
  Tensor u_com = sub(graph, var_one_matrix, u, prog,
             fPrefix + "/1-updateGate");
  // h_prev_c = h_prev - c
  auto h_prev_c = sub(graph, h_prev, c, prog,
             fPrefix + "/preOutput-candidate");
  // (1-u) * d_h, (h_prev - c) * d_h
  auto t =
    mul(graph, concat({u_com, h_prev_c}),
        d_h.broadcast(2, 0), prog, fPrefix + "/MulOGate");
  auto gradAtCandidateInput  = t.slice(0, outputGrad.dim(0));
  auto gradAtUpdateGateInput = t.slice(outputGrad.dim(0),
                                       2 * outputGrad.dim(0));
  ...
  if (weightsInput == nullptr) {
    w_ru = flattenUnits(
                  concat(weightsOutput[0], weightsOutput[1], 1)).transpose();
    w_c  = flattenUnits(weightsOutput[2]).transpose();
  } else {
    w_ru = concat(concat((*weightsInput)[0], weightsOutput[0], 0),
                concat((*weightsInput)[1], weightsOutput[1], 0), 1).transpose();
    w_c =  concat((*weightsInput)[2], weightsOutput[2], 0).transpose();
  }
  w_c =
      preArrangeMatMulInputRHS(graph, d_c.shape(), w_c,
                               initProg, fPrefix + "/PreArrangeWeights C",
                               mmOpt, cache);
  ...
  if (weightsInput) {
    auto out =
      matMul(graph, d_c, w_c, prog,
             fPrefix + "/d_x2_d_h_prevr", mmOpt, cache);
    ...
    auto out =
      matMul(graph, d_c, w_c, prog, fPrefix + "/PrevStepGrad",
             mmOpt, cache);
    d_hr =
  ...
  Tensor d_r;
  {
    ...
    d_r = nonLinearityInputGradient(graph, NonLinearityType::SIGMOID,
                              r, t, prog, fPrefix + "/t * r * (1-r)");
  }
  ...
  debug_tensor(prog, "bwd d_u",      d_u);
  debug_tensor(prog, "bwd d_c",      d_c);
  return std::make_tuple(d_h_prev,
                         d_x,
                         concat({d_r.expand({0}),
                                 d_u.expand({0}),
                                 d_c.expand({0})}));
...
basicGruBackwardStep(Graph &graph,
                     const Tensor *gradNextLayer,
                     const Tensor &fwdIntermediates,
                     const Tensor &prevStepOut,
...
/// that is required.
static void
basicGruParamUpdate(Graph &graph,
                    const Tensor &prevLayerActs,
  ...
  matMulAcc(graph,
            ...
            1.0,
            x_h_prev.transpose(),
            flattenUnits(concat(d_r.expand({0}), d_u.expand({0}))),
            prog,
            fPrefix + "/dw for reset and update weight",
            mmOpt, cache);
  ...
  matMulAcc(graph,
            ...
            d_c,
            prog,
            fPrefix + "/dw for candidate weight",
            mmOpt, cache);
...
static GruWeights
basicGruParamUpdateFinal(Graph &graph,
                          const GruWeights &weights,
                          const GruWeights &weightGrads,
                          Sequence &prog,
                          const std::string &debugPrefix) {
...
/// backward pass.
static GruWeights
createWeightAccumulators(Graph &graph,
                         const GruWeights  &weights,
                         const Tensor      &bwdIntermediates,
...
static Tensor
gruBwdImpl(Graph &graph, const GruParams &params,
  ...
  Tensor fwdIntermediates = dynamicSlice(graph, fwdIntermediatesSeq, seqIdx,
  ...
  auto wuLoopBody = Sequence();
  {
    ...
    if (inputGradSeq) {
      ...
      *inputGradSeq =
      ...
      std::tie(newOutGrad, bwdIntermediates) =
    ...
    if (bwdIntermediatesPtr) {
      *bwdIntermediatesPtr =
        createOutputTensor(graph, params,
                           seqSize * BASIC_GRU_CELL_NUM_UNITS,
  ...
  if (weightsGrad) {
    prog.add(wuLoopBody);
    *weightsGrad = basicGruParamUpdateFinal(graph, weights, *weightsGrad,
                                             prog, debugPrefix);
  }
...
Tensor gruBwd(Graph &graph, const GruParams &params,
                  program::Sequence &prog,
                  const Tensor     &fwdOutputInit,
                  const Tensor     &fwdIntermediatesSeq,
                  const GruWeights &weights,
  ...
  return gruBwdImpl(graph, params, prog, fwdOutputInit, fwdIntermediatesSeq,
                     ...
                     inputGrad, bwdIntermediates,
                     nullptr, debugPrefix, std::move(options), planningCache);
}

static GruWeights
gruWUImpl(Graph &graph, const GruParams &params,
           program::Sequence &prog,
           const Tensor      &fwdOutputInit,
           const Tensor      &fwdIntermediatesSeq,
...
Tensor gruBwdWithWU(poplar::Graph             &graph,
                      const GruParams           &params,
  ...
  Tensor outGrads =
    gruBwdImpl(graph, params, prog, fwdOutputInit, fwdIntermediates, weights,
                input, output, outputGrad, inputGrad,
...
uint64_t getBasicGruCellFwdFlops(const GruParams &params) {
  ...
  uint64_t multsWeighInp =
      static_cast<uint64_t>(inputSize) * 3 * outputSize * batchSize *
                             sequenceSize * 2;

  uint64_t multsWeighOut =
      static_cast<uint64_t>(outputSize) * 3 * outputSize * batchSize *
      sequenceSize * 2;
...
uint64_t getBasicGruCellBwdFlops(const GruParams &params) {
  ...
  uint64_t inputGradFlops =
      calcInputGrad ?  static_cast<uint64_t>(inputSize) * 3 * outputSize *
                       batchSize * sequenceSize * 2 : 0;
  uint64_t outputGradFlops =
      static_cast<uint64_t>(outputSize) * 6 * outputSize * batchSize *
      sequenceSize * 2;
...
uint64_t getBasicGruCellWuFlops(const GruParams &params) {
  ...
  uint64_t weightFlops =
      static_cast<uint64_t>(inputSize + outputSize) * 3 * outputSize *

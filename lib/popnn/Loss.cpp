...
maxMinArgMaxMin(Graph &graph, const Tensor &input,
                const boost::optional<Type> &indexType,
                const boost::optional<unsigned> valueTile, Sequence &prog,
                bool maxFlag, const DebugNameAndId &dnai) {
  auto inShape = input.shape();
  std::vector<std::size_t> outShape{inShape.begin(), inShape.end() - 1};
  auto [max, indices] = popops::topKWithPermutation(
      graph, prog, input, {1, maxFlag, popops::SortOrder::NONE}, dnai);
  auto maxVal = max;
  if (valueTile) {
    maxVal = graph.addVariable(max.elementType(), outShape, {dnai, "output"});
    graph.setTileMapping(maxVal, *valueTile);
    prog.add(Copy(max, maxVal, false, {dnai}));
  }
  if (indexType && (indexType != indices.elementType())) {
    indices = popops::cast(graph, indices, *indexType, prog, {dnai});
  }
  return std::make_pair(maxVal, indices);
}
...
maxAndArgMaxImpl(Graph &graph, const Tensor &input, Sequence &prog,
  ...
  return maxMinArgMaxMin(graph, input, {}, numCorrectTile, prog, maxFlag,
                         {dnai});
}
...
Tensor argMax(Graph &graph, const Tensor &input, Sequence &prog,
  ...
  auto output = maxAndArgMaxImpl(graph, input, prog, true, {di}).second;
  di.addOutput(output);
  return output;
}
...
maxAndArgMax(Graph &graph, const Tensor &input, Sequence &prog,
  ...
  logging::popnn::info("maxAndArgMax input={}, name={}", input.shape(),
                       debugContext.getPathName());
  auto [values, indices] = maxAndArgMaxImpl(graph, input, prog, true, {di});
...
Tensor argMin(Graph &graph, const Tensor &input, Sequence &prog,
  ...
  auto results = maxAndArgMaxImpl(graph, input, prog, false, {di});
  di.addOutput(results.second);
...
minAndArgMin(Graph &graph, const Tensor &input, Sequence &prog,
  ...
  logging::popnn::info("minAndArgMin input={}, name={}", input.shape(),
                       debugContext.getPathName());
  auto [values, indices] = maxAndArgMaxImpl(graph, input, prog, false, {di});
...
Program calcAccuracy(Graph &graph, const Tensor &modelOutputs,
  ...
  Sequence prog({}, {di});
  auto maxIndices =
      maxMinArgMaxMin(graph, modelOutputs, expected.elementType(),
                      *numCorrectTile, prog, true, {di, layerPrefix})
          .second;

...
matMul1(Graph &graph, Tensor A, Tensor B, Sequence &prog,
  ...
  for (unsigned b = 0; b < bCols; ++b) {
          ...
          for (unsigned i = tileRowBegin; i != tileRowEnd; ++i) {
            ...
            auto v =
                graph.addVertex(dotProductCS,
                                templateVertex("popnn::MatMul1Partial",
                                               dType),
                                {{"in", partialIn},
    ...
    popstd::reduce(graph, partials, out[b], graph.getTileMapping(out[b]),
                   reduceCS);
  }
  return out;
}
          ...
          for (unsigned i = beginElement; i < endElement; i += vectorWidth) {
            ...
            if (tileRowBegin == tileRowEnd) {
              ...
              auto v = graph.addVertex(
                            cs,
                            templateVertex("popnn::MatMul2", dType),
                            {{"in", inWindow},
                             {"weights", w},
                             {"out", outWindow},
                             });
      ...
      for (unsigned tileX = 0; tileX != ipuPartition.tilesPerRow; ++tileX) {
        ...
        for (unsigned i = tileRowBegin; i != tileRowEnd; ++i) {
          ...
          auto aWindow = A.slice({i, 0}, {i + 1, aCols})
                          .flatten();
          auto vertexTypeName = update ? "popnn::MatMul3Update"
                                       : "popnn::MatMul3";
...
matMul(poplar::Graph &graph, poplar::Tensor &C, bool isUpdate, float k,
       ...
       const MatMulOptions &options) {
  if (A.dim(1) != B.dim(0)) {
    throw popnn::popnn_error("Second dimension of first operand to matrix "
  ...
  if (bIsVector && !aIsVector) {
    if (isUpdate) {
        throw popnn::popnn_error("Matrix * vector matMulAcc "
                                 "not implemented");
    }
    if (aColCont && options.outputIsRowContiguous) {
      ...
      throw popnn::popnn_error("Matrix (col contiguous) * vector for "
      ...
      throw popnn::popnn_error("Matrix (row contiguous) * vector for "
                               "col contiguous output not implemented");
    }
  }
  if (aIsVector && !bIsVector) {
    if (isUpdate) {
      throw popnn::popnn_error("Vector * matrix matMulAcc "
                               "not implemented");
    }
    if (bColCont && !options.outputIsRowContiguous) {
      ...
      throw popnn::popnn_error("Vector * matrix (col contiguous) "
                               "for row contiguous output not implemented");
    } else if (!options.outputIsRowContiguous) {
      return matMul1(graph, B.transpose(), A.transpose(), prog,
                     debugPrefix, options);
    } else {
      throw popnn::popnn_error("Vector * matrix (row contiguous) "
                               "for row contiguous output not implemented");
    }
  }
  if (aIsVector && bIsVector) {
    throw popnn::popnn_error("Dot product matmul not implemented yet");
  }
  if (aColCont && bRowCont) {
    if (isUpdate) {
        throw popnn::popnn_error("Matrix (col contig) * matrix (row contig) "
  ...
  if (aColCont && bColCont) {
    if (isUpdate) {
      throw popnn::popnn_error("Matrix (col contig) * matrix (col contig) "
                               "matMulAcc not implemented");
    }
    if (options.outputIsRowContiguous) {
      throw popnn::popnn_error("Matrix (col contig) * matrix (col contig) "
  ...
  throw popnn::popnn_error("matmul for input layout not implemented yet");
}

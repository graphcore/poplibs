...
// [N][....F..][C]
static bool canFlattenDim(const poplin::ConvParams &params,
                          unsigned dim) {
...
static Transform getTransform(const poplar::Graph &graph,
  ...
  if (numChans < chanGrainSize) {
    ...
    const auto desiredFactor =
      lcm(numChans, chanGrainSize) / numChans;
    ...
    for (const auto &entry : groupings) {
      ...
      if (canFlattenDim(params, d) && transformedDims.count(d) == 0 &&
        ...
        const auto f = gcd(desiredFactor / currentFactor,
                           gcd(in.dim(d), grouping));
  ...
  f = [&](std::size_t tiles, std::size_t fieldIndex) {
    for (std::size_t elem = 1UL;
      ...
      if (fieldIndex + 1 == fieldShape.size()) {
        const auto thisCost = computeCost(transformedParams,
                                          split, vectorWidth,
                                          chansPerGroupDet,
                                          target);
        if (thisCost < cost) {

...
ScaledAddEstimates modelContiguousScaledAdd(
    const Target &target, const Type &dataType, const Type &dataBType,
  ...
  e.cycles = m.call<unsigned>(
      {mNumElems},
      [targetParams, dataType, dataBType,
       isMemConstrained](const std::vector<unsigned> &values) {
        ...
        const auto cycles = getScaledArithmeticSupervisorCycleEstimate(
            targetParams, dataType, dataBType, isMemConstrained,
            ScaledArithmeticOp::ADD, vectorLayout, vectorLayout, numElems);

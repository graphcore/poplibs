...
std::vector<std::vector<unsigned int>> HyperGraph::populateNodesC(
  ...
  return populateDataNodes(nRowC, nColC, blockIdMatrixC, nodeC, edgeC);
}
...
void HyperGraph::createComputeSetMatMul(
  ...
  for (const auto &n : nodeV) {
    ...
    if (partialDataType == poplar::FLOAT) {
      graph.setInitialValue(v["transformedOutStride"], 2);
    } else {
      graph.setInitialValue(v["transformedOutStride"], 4);
    }

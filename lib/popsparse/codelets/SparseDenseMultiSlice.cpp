...
public:
  ...
  bool compute() {
    ...
    const auto function = computeSlice<FPType, BaseTNZType, SubTType>;
...
template <typename FPType, typename BaseTNZType, typename SubTType>
static bool computeUpdate(Input<Vector<unsigned>> &offsets,
                          BaseTNZType &baseTNZ,
                          BaseTMetaInfoType &baseTMetaInfo, SubTType &subT,
...
public:
  ...
  Input<MetaInfoType> yPartitionToProcess;
  Input<FPType> scale;

  bool compute() {

    const auto function = computeUpdate<FPType, BaseTNZType, SubTType>;

    return function(offsets, baseTNZ, baseTMetaInfo, subT, rowsPerPartition,
                    *yPartitionToProcess, nzScaleFactor, subColumns, *scale);
  }
};

...
static void TestFunc(poplar::TensorCloneMethod cloneMethod) {
  ...
  auto srcTensor =
         cloneMethod == poplar::TensorCloneMethod::PRESERVE_ORDER_AND_ALIASES ?
            testInput.broadcast(broadcastFactor, 0) : testInput;

  // test funtion: duplicate (clone+copy) the tensor
  Tensor testResult = poputil::duplicate(graph, srcTensor,
                            prog, "testResult", cloneMethod);

  // create unaliased view of the testResult
  Tensor testResultFlat = testResult.flatten();
  auto testResultFlatRegions =
         graph.getSortedContiguousRegions(testResultFlat,
                                          {{0, testResultFlat.numElements()}},
                                          true);
  Tensor testResultNA = poplar::concat(testResultFlat
                                   .slices(testResultFlatRegions));
  ...
  auto rawHostTestResult =
    allocateHostMemoryForTensor(testResultNA, "testResultNA", graph,
  ...
  auto GetVecVecIntervals = [&graph](const Tensor& t, TyVecA& aliases)
                              -> TyVecVecI {
         return graph.getSortedContiguousRegions(t.flatten(),
                                            {{0, t.numElements()}},
                                            false, &aliases);
       };

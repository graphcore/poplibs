...
static void iteratePadding(const Graph &graph,
                           const Tensor &tIn,
                           const Tensor &tOut,
                           const std::vector<ptrdiff_t> &pLows,
                           const std::vector<ptrdiff_t> &pUpps,
...
void checkMapping(const Graph &graph,
                  const Tensor &tIn, const Tensor &tOut,
                  const std::vector<ptrdiff_t> &pLows,
                  const std::vector<ptrdiff_t> &pUpps,
  ...
  switch (mappingMethod) {
    case padding::MappingMethod::NONE:
      iteratePadding(graph, tIn, tOut, pLows, pUpps,
        [&](const Tensor &tStripped,
            const Tensor &lower, const Tensor &upper, unsigned dim) {
    ...
    case padding::MappingMethod::ZERO:
      iteratePadding(graph, tIn, tOut, pLows, pUpps,
        [&](const Tensor &tStripped,
          ...
          auto mapping = graph.getTileMapping(padding, false);
          auto t0MappedElems =
            std::accumulate(mapping[0].begin(), mapping[0].end(),
                            std::size_t(0),
    ...
    case padding::MappingMethod::EDGE:
      iteratePadding(graph, tIn, tOut, pLows, pUpps,
        [&](const Tensor &tStripped,
            const Tensor &lower, const Tensor &upper, unsigned dim) {
          auto expectedLowerMapping =
            graph.getTileMapping(tStripped.slice(0, 1, dim)
                                          .broadcast(lower.dim(dim), dim));
          auto expectedUpperMapping =
            graph.getTileMapping(tStripped.slice(tStripped.dim(dim) - 1,
                                                 tStripped.dim(dim), dim)

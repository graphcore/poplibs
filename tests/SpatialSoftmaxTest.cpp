...
BOOST_AUTO_TEST_CASE(SpatialSoftmax) {
  ...
  // (NOTE: we disabled the softmax normalisaiton above, otherwise the expected
  // result is painful to compute!)
  ...
  const std::vector<float> expected = {1.f/2, -2.f/3}; // {ey, ex} => {row, col}

...
static bool channelMulTests(const std::vector<TestCase> &cases) {
  ...
  for (std::size_t i = 0; i < cases.size(); ++i) {
    ...
    std::cout << "Test case [" << i << "]: "
              << " scaleLen: " << tc.scaleLen << " actsLen: " << tc.actsLen
              << " type: " << tc.type.toString() << "\n";

    std::string suffix = "_" + std::to_string(i);

    auto scale = graph.addVariable(tc.type, {tc.scaleLen}, "scale" + suffix);
    auto actsIn = graph.addVariable(tc.type, {tc.actsLen}, "actsIn" + suffix);
    ...
    auto v =
        graph.addVertex(cs, templateVertexName,
                        {{"data", actsIn}, {"out", actsOut}, {"B", scale}});
    ...
    tcData[i].rawScale = allocateHostMemoryForTensor(
        scale, "scale" + suffix, graph, uploadProg, downloadProg, tmap);
    tcData[i].rawActsIn = allocateHostMemoryForTensor(
        actsIn, "acts" + suffix, graph, uploadProg, downloadProg, tmap);
    tcData[i].rawActsOut = allocateHostMemoryForTensor(
        actsOut, "out" + suffix, graph, uploadProg, downloadProg, tmap);

    tcData[i].scale.resize(tc.scaleLen);
    tcData[i].actsIn.resize(tc.actsLen);

    std::mt19937 randomEngine;
    writeRandomValues(target, scale.elementType(), tcData[i].scale.data(),
                      tcData[i].scale.data() + tcData[i].scale.size(), -2., 2.,
                      randomEngine);
    writeRandomValues(target, actsIn.elementType(), tcData[i].actsIn.data(),
                      tcData[i].actsIn.data() + tcData[i].actsIn.size(), -2.,
                      2., randomEngine);
    ...
    copy(target, ones.data(), ones.size(), actsOut.elementType(),
         tcData[i].rawActsOut.get());
  }
  ...
  for (std::size_t i = 0; i < tcData.size(); ++i) {
    ...
    copy(target, tc.type, tcData[i].rawActsOut.get(), actsOut.data(),
         actsOut.size());
    ...
    auto matchesModel =
        checkIsClose("out", actsOut.data(), {actsOut.size()}, actsOutRef.data(),
...
BOOST_AUTO_TEST_CASE(ChannelMulTiny) {
  std::vector<TestCase> cases = {
      {HALF, 1, 8},  {HALF, 4, 16},  {HALF, 8, 32},  {HALF, 5, 15},
      {FLOAT, 1, 8}, {FLOAT, 4, 16}, {FLOAT, 8, 32}, {FLOAT, 5, 15},
  };
...
BOOST_AUTO_TEST_CASE(ChannelMulSmall,
  ...
  std::vector<TestCase> cases = {
      ...
      {HALF, 5, 15},    {HALF, 8, 168},

      {FLOAT, 1, 480},  {FLOAT, 4, 480}, {FLOAT, 8, 480}, {FLOAT, 12, 480},
      {FLOAT, 16, 480}, {FLOAT, 1, 15},  {FLOAT, 4, 12},  {FLOAT, 8, 40},
      {FLOAT, 5, 15},   {FLOAT, 8, 168},
  };
  BOOST_TEST(channelMulTests(cases));
}

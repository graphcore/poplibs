...
executeExpr(const DeviceType &deviceType, const Expr &expression,
  ...
  rawOutOpt = allocateHostMemoryForTensor(inPlace ? a : tOpt, "rawOpt", g,
                                          uploadProg, downloadProg, tmap);
  rawOutNoOpt = allocateHostMemoryForTensor(inPlace ? c : tNoOpt, "rawNoOpt", g,
                                            uploadProg, downloadProg, tmap);

  Sequence controlProg(
      {std::move(uploadProg), std::move(prog), std::move(downloadProg)});
...
int main(int argc, char **argv) {
  ...
  auto mainExpr = Add(Sub(Add(_1, _2), _1), _2);
  auto [opt, noOpt] = executeExpr(deviceType, mainExpr, dataType, inPlace,

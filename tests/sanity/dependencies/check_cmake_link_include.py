...
def all_lib_files(lib, base_path):
    files = []
    files += glob.glob(base_path + "/lib/" + lib + "/**/*.*pp", recursive=True)
    files += glob.glob(base_path + "/include/" + lib +
                       "/**/*.*pp", recursive=True)
    return files
...
def get_include_deps_for_lib(lib, files):
    ...
    for file_name in files:
        with open(file_name) as file:
            ...
            for line in file:
                match = re.search("#\s*include [<,\"](pop.*?)/", line)
                if match:
                    dep = match.group(1)
                    if dep != lib:
...
def main():
    ...
    for lib in libs:
        for dep in declared_dependencies[lib]:
            if dep not in include_dependencies[lib] and dep.startswith("pop"):
                logging.warn(

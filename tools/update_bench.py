...
from bench import read_results_file, CHANGED_RESULT_PATTERN, TestKey, Expected, TestFailureException
...
RUNNING_PROGRESS = re.compile(r'^\s+Start (\d+): ([a-zA-Z]+)(2?)_default_(.+)_benchmark$')

RESULT_PATTERN = re.compile(r"\s*\d+/\d+ Test\s+#(\d+): ([\w.]+) \.+[\*\s]+(\w+)\s+([\d\.]+) sec")
"""
Matches lines like:
  1/414 Test #1775: IpuModel_default_gemm_200x64x10000_benchmark .............................   Passed    3.95 sec
"""
...
def updated_results_iter(cmd, number_of_benchmarks):
    ...
    with BenchmarksBar('Running', max=number_of_benchmarks) as prog_bar, \
            subprocess.Popen(cmd, stdout=subprocess.PIPE) as proc:
        for line in proc.stdout:
            dline = line.decode('utf-8')
            progress_match = RUNNING_PROGRESS.match(dline)
            if progress_match:
                prog_bar.next()
                continue
            ...
            result_match = RESULT_PATTERN.match(dline)
            if result_match:
                _bench_num, bench_name, result, _duration = result_match.groups()
                if result != 'Passed':
                    proc.terminate()
                    print(f"\n\nERROR: '{bench_name}' has result '{result}'", file=sys.stderr)
